...
anes_subset02 <- na.omit(anes_subset02)
...
#new variable: age 
anes_subset02$age <- -1
anes_subset02$age[anes_subset02$V161267 >= 18] <- "18-30"
anes_subset02$age[anes_subset02$V161267 >= 30] <- "30-39"
anes_subset02$age[anes_subset02$V161267 >= 40] <- "40-49"
anes_subset02$age[anes_subset02$V161267 >= 50] <- "50-59"
...
#new variable: political party 
anes_subset02$party <- -1
anes_subset02$party[anes_subset02$V161019== 1] <- "dem"
anes_subset02$party[anes_subset02$V161019== 2] <- "rep"
anes_subset02$party[anes_subset02$V161019== 3] <- "ind_other"
anes_subset02$party <- factor(anes_subset02$party)

#new variable: political interest 
anes_subset02$interest <- -1
anes_subset02$interest[anes_subset02$V161004== 1] <- "low"
anes_subset02$interest[anes_subset02$V161004== 2] <- "moderate"
anes_subset02$interest[anes_subset02$V161004== 3] <- "high"
anes_subset02$interest <- factor(anes_subset02$interest)
...
output1 <- lm(primary2016 ~ insurance, data = anes_subset03)
summary(output1)

output2 <- lm(primary2016 ~ V161112 + V161277 + V161267 + V161113 + V161270 + V161307 + V161342, data = anes_subset03)
summary(output2)

output3 <- lm(primary2016 ~ insurance + job + age + ACA + edu + socio + sex, data = anes_subset03)
summary(output3)
